...
from webdriver_utility import CustomWebDriverManager
...
class UIActions(CustomWebDriverManager):
    ...
    def click_partial_link(cls, partial_text: str):
        element = cls.driver.find_element(By.PARTIAL_LINK_TEXT, partial_text)
        element.click()
    ...
    def _find_elements(cls, locator: str):
        ...
        elements = cls.driver.find_elements(by, locator)
        return elements
    ...
    def execute_keyboard_shortcut(cls, key: str, modifier_key: str = None):
        actions = ActionChains(cls.driver)
        if modifier_key:
            actions.key_down(getattr(Keys, modifier_key.upper())).send_keys(key).key_up(getattr(Keys, modifier_key.upper()))
        else:
    ...
    def scroll_to_view(cls, locator: str, index: int = 0, scroll_options: dict = None):
        ...
        cls.driver.execute_script(script, element)
    ...
    def is_element_focused(cls, locator: str, index: int = 0, scroll_options: dict = None):
        try:
            element = cls._find_element(locator, index=index, scroll_options=scroll_options)
            return element == cls.driver.switch_to.active_element
        except NoSuchElementException:
            return False
...
class UIWaits(CustomWebDriverManager):
    ...
    def wait_until_visible(cls, locator: str, timeout: int = DEFAULT_TIMEOUT, ignore_timeout: bool = False):
        by = By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR
        try:
            WebDriverWait(cls.driver, timeout).until(EC.presence_of_element_located((by, locator)))
            WebDriverWait(cls.driver, timeout).until(EC.visibility_of_element_located((by, locator)))
        except TimeoutException:
            if ignore_timeout:
    ...
    def wait_until_clickable(cls, locator: str, timeout: int = DEFAULT_TIMEOUT, ignore_timeout: bool = False):
        by = By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR
        try:
            WebDriverWait(cls.driver, timeout).until(EC.element_to_be_clickable((by, locator)))
        except TimeoutException:
            if ignore_timeout:
    ...
    def wait_until_stale(cls, locator: str, timeout: int = DEFAULT_TIMEOUT):
        try:
            element = cls.driver.find_element(By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR, locator)
            WebDriverWait(cls.driver, timeout).until(EC.staleness_of(element))
    ...
    def wait_until_invisible(cls, locator: str, timeout: int = DEFAULT_TIMEOUT):
        try:
            element = cls.driver.find_element(By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR, locator)
            WebDriverWait(cls.driver, timeout).until(EC.invisibility_of_element(element))
        except (NoSuchElementException, AttributeError, TimeoutException):
            print(f'Element "{locator}" did not become invisible within the timeout period')

    @classmethod
    def wait_until_text_present(cls, locator: str, text: str, timeout: int = DEFAULT_TIMEOUT, ignore_timeout: bool = False):
        by = By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR
        try:
            WebDriverWait(cls.driver, timeout).until(EC.text_to_be_present_in_element((by, locator), text))
        except TimeoutException:
            if ignore_timeout:
    ...
    @classmethod
    def wait_until_value_present(cls, locator: str, value: str, timeout: int = DEFAULT_TIMEOUT, ignore_timeout: bool = False):
        by = By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR
        try:
            WebDriverWait(cls.driver, timeout).until(EC.text_to_be_present_in_element_value((by, locator), value))
        except TimeoutException:
            if ignore_timeout:
...
class BrowserActions(CustomWebDriverManager):
    ...
    def switch_to_frame(cls, locator: str):
        element = cls.driver.find_element(By.XPATH if locator.startswith(XPATH_PREFIXES) else By.CSS_SELECTOR, locator)
        cls.driver.switch_to.frame(element)
    ...
    def retrieve_cookie(cls, name: str):
        cookie = cls.driver.get_cookie(name)
        if cookie:
            return cookie
        else:
    ...
    def save_screenshot(cls, file_path: str, format: str = 'png'):
        ...
        cls.driver.save_screenshot(f'{file_path}.{format}')

    @classmethod
    def switch_to_window_by_index(cls, index: int):
        try:
            cls.driver.switch_to.window(cls.driver.window_handles[index])
        except IndexError:
